...
func TimeMeasurement(sortName string, seed int64) (arrX []int, arrY []int) {
	var execTime int
...
func measuringSelectionSort(arr []int) int {
	timeStart := time.Now()
	sorts.SelectSort(arr)
	timeEnd := time.Now()
	timeFinally := timeEnd.Sub(timeStart)
	Time := int(timeFinally.Nanoseconds())
	return Time
}

func measuringInsertionSort(arr []int) int {
	timeStart := time.Now()
...
func measuringQuickSort(arr []int) int {
	timeStart := time.Now()
